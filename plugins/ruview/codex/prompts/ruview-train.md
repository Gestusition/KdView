...
- **snn** (local env adaptation, <30 s): `node scripts/snn-csi-processor.js --port 5006`; the historical gateway tutorial in `docs/tutorials/cognitum-seed-pretraining.md`; ADR-084/085 (RaBitQ), ADR-086 (novelty gate).
- **gpu**: set your own `GCP_PROJECT`, run `gcloud auth login`, then `bash scripts/gcloud-train.sh --dry-run` (smoke), `bash scripts/gcloud-train.sh --gpu l4 --hours 2` (proto), `bash scripts/gcloud-train.sh --gpu a100 --config scripts/training-config-sweep.json`, or `bash scripts/gcloud-train.sh --sweep`. VM auto-deletes unless `--keep-vm`. Local Mac: `bash scripts/mac-mini-train.sh`. Bench: `python scripts/benchmark-model.py`.
