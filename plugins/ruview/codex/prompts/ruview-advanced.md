...
- **field-model** (ADR-030) — `ruvsense/field_model.rs` builds an SVD eigenstructure of the room and persists it as RVF locally or on a compatible operator-managed gateway; new frames are projected against it and the residual is the perturbation. Survives restarts; answers "what's different from the empty-room baseline?"
