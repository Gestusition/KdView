...
3. Run:
   ...
   python firmware/esp32-csi-node/provision.py --port <PORT> \
     ...
     [--gateway-url http://… --gateway-token … --zone lobby] [--swarm-hb 30] [--swarm-ingest 5] [--dry-run]
   ```
4. Explain trade-offs: `--channel` pins the node (AP's channel) vs. `--hop-channels` for ADR-061 multi-freq hopping; `--filter-mac` restricts to one transmitter vs. omit for all (more data, more noise); `--edge-tier` 0/1/2 = off/stats/vitals; `--tdm-slot`/`--tdm-total` slot a multi-node mesh. The gateway is optional; `--seed-url` and `--seed-token` remain migration aliases for existing deployments.
