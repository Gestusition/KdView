#!/usr/bin/env python3
"""
ADR-069: ESP32 CSI → operator-managed RVF gateway bridge

Listens for CSI feature vectors from ESP32 nodes via UDP, batches them,
and ingests into a compatible RVF vector store via HTTP(S) REST API.

Usage:
    # Run bridge (default mode)
    python scripts/gateway_csi_bridge.py \
        --gateway-url http://127.0.0.1:8080 \
        --token "$RUVIEW_GATEWAY_TOKEN" \
        --udp-port 5006 \
        --batch-size 10
    ...
    # Print gateway stats
    python scripts/gateway_csi_bridge.py --token TOKEN --stats

    # Trigger store compaction
    python scripts/gateway_csi_bridge.py --token TOKEN --compact
...
class GatewayClient:
    """HTTP(S) client for a compatible operator-managed gateway."""
...
def _flush_batch(gateway: GatewayClient, batch: list, stats: dict,
                 validate: bool = False, validation_stats: dict | None = None,
                 last_features: list[float] | None = None) -> None:
    """Ingest a batch of vectors into the gateway, with optional retry and validation."""
    max_attempts = 2
    for attempt in range(max_attempts):
        try:
            result = gateway.ingest(batch)
    ...
    if validate and last_features is not None and validation_stats is not None:
        _run_validation(gateway, last_features, validation_stats)


def _run_validation(gateway: GatewayClient, features: list[float],
                    validation_stats: dict) -> None:
    """Query kNN for the most recent vector and compare with PIR sensor."""
    try:
        qr = gateway.query(features, k=1)
...
def run_bridge(args):
    ...
    gateway = GatewayClient(args.gateway_url, args.token)

    # Verify connectivity
    try:
        status = gateway.status()
        log.info(
            "Connected to gateway %s — %d vectors, epoch %d, dim %d",
            status["device_id"][:8],
        ...
        log.error("Cannot connect to gateway at %s: %s", args.gateway_url, e)
        sys.exit(1)
        ...
        while True:
            try:
                ...
                if batch and (now - last_flush) >= args.flush_interval:
                    _flush_batch(gateway, batch, stats, args.validate,
...
def run_stats(args):
    """Query the gateway and print comprehensive stats."""
    gateway = GatewayClient(args.gateway_url, args.token)
...
def run_compact(args):
    """Trigger store compaction on the gateway."""
    gateway = GatewayClient(args.gateway_url, args.token)
    print("Triggering store compaction...")
    try:
        result = gateway.compact()
...
def main():
    parser = argparse.ArgumentParser(
        description="ADR-069: ESP32 CSI -> operator-managed RVF gateway bridge"
    )
    parser.add_argument(
        "--gateway-url", "--seed-url",
        dest="gateway_url",
        default=os.environ.get("RUVIEW_GATEWAY_URL", "http://127.0.0.1:8080"),
        help="Compatible gateway URL (legacy --seed-url alias is accepted)",
    )
    parser.add_argument(
        "--token",
        default=os.environ.get("RUVIEW_GATEWAY_TOKEN") or os.environ.get("SEED_TOKEN"),
        help="Gateway Bearer token (or set RUVIEW_GATEWAY_TOKEN)",
    )
    parser.add_argument(
        "--udp-port",
        type=int,
    ...
    parser.add_argument(
        "--stats",
        action="store_true",
        help="Print gateway stats (vectors, boundary, coherence, graph) and exit",
    )
    parser.add_argument(
        "--compact",
        action="store_true",
    ...
    if not args.token:
        parser.error("--token is required (or set RUVIEW_GATEWAY_TOKEN)")
